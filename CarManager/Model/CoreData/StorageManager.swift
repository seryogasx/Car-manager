...
let carGateway = CarStorageCateway(storageManager: storageManager)
...
protocol CarsDelegate: NSFetchedResultsControllerDelegate {
...
class StorageManager: StorageManagerProtocol {
    ...
    lazy var persistentContainer: NSPersistentContainer = {
        ...
        container.loadPersistentStores { _, error in
            if let error = error {
                print("Fail to load persistentContainer! \(error)")
            }
        }
        return container
    }()
    ...
    func saveContext(context: NSManagedObjectContext, completion: @escaping (StorageError?) -> Void) {
        if context.hasChanges {
            do {
                ...
                completion(StorageError.failToSave(message: "Fail to save context! \(error.localizedDescription)"))
            }
        } else {
            completion(StorageError.nothingToSave(message: "Nothing to save into \(context.description)"))
        }
    }
...
//        items.append("aidKitAge")
    ...
    static var propertiesCount: Int {
        guard let entity = NSEntityDescription.entity(forEntityName: "Car", in: storageManager.mainContext) else { return 0 }
        let someCar = Car(entity: entity, insertInto: storageManager.mainContext)
        return someCar.entity.attributesByName.count
    }

    static var paramNames: [String] {
        guard let entity = NSEntityDescription.entity(forEntityName: "Car", in: storageManager.mainContext) else { return [] }
        let someCar = Car(entity: entity, insertInto: storageManager.mainContext)
